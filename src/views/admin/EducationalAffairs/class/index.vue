...
import { queryVocationalById, queryVocationalByClassId } from "@/api/vocational";
...
export default {
  data () {
    return {
      ...
      classInfoRules: {
        ...
        classStr: [{ required: true, message: '请输入班级名称', trigger: 'blur' }],
        counselorId: [
  ...
  methods: {
    ...
    async getClass () {
      ...
      if (res.code == 200) {
        ...
        for (let i = 0; i < temp.length; i++) {
          temp[i].collegeId = await this.getCollegeEnum({ classId: temp[i].collegeId });
          temp[i].vocationalId = await this.getVocationalEnum(
            { classId: temp[i].vocationalId }
          );
          temp[i].counselorId = await this.getCounselorEnum({ classId: temp[i].counselorId });
        }
        this.tableData = temp
      }
    ...
    async getCollegeList () {
      ...
      this.collegeList = res.data.result ? res.data.result.map((x) => ({
        ...x,
        label: x.collegeStr,
        value: x.id,
      })) : [];
    ...
    async getVocationalList (row) {
      ...
      this.vocationalList = res.data ? res.data.map((x) => ({
    ...
    async getCounselorList (row) {
      ...
      this.counselorList = res.data ? res.data.map((x) => ({
        label: x.name,
        value: x.id,
      })) : [];
    },
